...
namespace MonHunCollabRestored.Character
{
    public class CH092_Controller : CharacterControlBase
    {
        ...
        private void InitLinkSkill()
        {
            ...
            if (this._tSkill1_LinkSkill == null && this._refEntity.PlayerSkills[1].BulletData.n_LINK_SKILL != 0)
            {
                ...
                if (ManagedSingleton<OrangeDataManager>.Instance.SKILL_TABLE_DICT.TryGetNewValue(weaponStruct2.BulletData.n_LINK_SKILL, out this._tSkill1_LinkSkill))
                {
                    ...
                    CollideBullet collideBullet = gameObject.AddComponent<CollideBullet>();
                    gameObject.name = this._tSkill1_LinkSkill.s_MODEL;
                    gameObject.layer = ManagedSingleton<OrangeLayerManager>.Instance.BulletLayer;
                    MonoBehaviourSingleton<PoolManager>.Instance.CreatePoolBaseLocal<CollideBullet>(collideBullet, this._tSkill1_LinkSkill.s_MODEL, 5);
                }
            }
        }
        ...
        public override void CreateSkillBullet(WeaponStruct wsSkill)
        {
            ...
            if (curMainStatus == OrangeCharacter.MainStatus.SKILL)
            {
                ...
                if (curSubStatus == OrangeCharacter.SubStatus.SKILL0_1)
                {
                    ...
                    SKILL_TABLE skill_TABLE = wsSkill.FastBulletDatas[0];
                    ...
                    bulletCollider.UpdateBulletData(skill_TABLE, sPlayerName, nowRecordNO, num, this._refEntity.direction);
                ...
                if (curSubStatus == OrangeCharacter.SubStatus.SKILL1)
                {
                    ...
                    SKILL_TABLE skill_TABLE2 = wsSkill.FastBulletDatas[0];
                    ...
                    bulletCollider2.UpdateBulletData(skill_TABLE2, sPlayerName2, nowRecordNO2, num, this._refEntity.direction);
                    this._refEntity.BulletCollider.SetBulletAtk(wsSkill.weaponStatus, this._refEntity.selfBuffManager.sBuffStatus, null);
                    this._refEntity.BulletCollider.BulletLevel = wsSkill.SkillLV;
                    this._refEntity.BulletCollider.Active(this._refEntity.TargetMask);
                    this._refEntity.BulletCollider.HitCallback = (CallbackObj)new System.Action<Il2CppSystem.Object>((Il2CppSystem.Object obj) =>
                    {
                        if (obj == null) { Plugin.Log.LogError("Hit Callback is null"); }
                        else
                        {
                            this.Skill1FlyKickHitCB(obj);
                        }
                    });
                ...
                switch (curSubStatus)
                {
                    case OrangeCharacter.SubStatus.SKILL0:
                        if (this.bPlaySkillFx && this._refEntity.CurrentFrame > 0.28f)
                        {
                            ...
                            Vector3 vector = this._refEntity.ModelTransform.position + new Vector3(0f, 0.67f, 0f);
                            Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_spiritslash_000", vector, OrangeCharacter.NormalQuaternion);
                        }
                        if (this.bPlaySkillFx2 && this._refEntity.CurrentFrame > 0.5f)
                        {
                            this.bPlaySkillFx2 = false;
                            Vector3 vector2 = this._refEntity.ModelTransform.position + new Vector3(0f, 0.67f, 0f);
                            Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_spiritslash_001", vector2, OrangeCharacter.NormalQuaternion);
                            return;
                        }
                        if (this._refEntity.CurrentFrame > 0.55f)
                        {
                            ...
                            Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_spiritslash_002", this._refEntity.ModelTransform, OrangeCharacter.NormalQuaternion);
                            this._refEntity.Animator._animator.speed = 1f;
                    ...
                    case OrangeCharacter.SubStatus.SKILL0_2:
                        ...
                        {
                            int num = Mathf.RoundToInt((float)((long)OrangeCharacter.WalkSpeed - (long)(OrangeCharacter.WalkSpeed / 500) * this._otSkill0Timer.GetMillisecond())) * this._refEntity.direction;
                            this._refEntity.SetSpeed(num, 0);
                            return;
                        }
                        this._refEntity.IgnoreGravity = false;
                        this._refEntity.SetSpeed(0, 0);
                    ...
                    default:
                        switch (curSubStatus)
                        {
                            ...
                            case OrangeCharacter.SubStatus.SKILL1_3:
                                ...
                                {
                                    this.bInSkill = false;
                                    Vector3 vector3 = this._refEntity.ModelTransform.position + new Vector3((float)this._refEntity.direction, -1f, 0f);
                                    Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_soaringkick_000", vector3, OrangeCharacter.NormalQuaternion);
                                    vector3 = this._refEntity.ModelTransform.position + new Vector3((float)this._refEntity.direction, 0f, 0f);
                                    Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_soaringkick_001", vector3, OrangeCharacter.NormalQuaternion);
                                    this.CreateSkillBullet(this._refEntity.PlayerSkills[1]);
                                    return;
                                }
        ...
        private void Skill1FlyKickHitCB(object obj)
        {
            ...
            EnemyControllerBase enemyControllerBase = component.tLinkSOB.Cast<EnemyControllerBase>();
            if (enemyControllerBase)
            {
                this._refEntity.BulletCollider.BackToPool();
                this._refEntity.BulletCollider.HitCallback = null;
                this._refEntity.SetStatus(OrangeCharacter.MainStatus.SKILL, OrangeCharacter.SubStatus.SKILL1_1);
                Vector3 vector = this._refEntity.ModelTransform.position + new Vector3((float)this._refEntity.direction * 1.5f, 1f, 0f);
                Tangerine.Utils.Il2CppHelpers.FxManagerPlay("fxuse_soaringkick_002", vector, OrangeCharacter.NormalQuaternion);
            }
        }
